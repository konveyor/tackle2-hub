...
func Add(mgr manager.Manager, db *gorm.DB) error {
	...
	// Primary CR.
	err = cnt.Watch(
		&source.Kind{Type: &v1alpha1.Addon{}},
...
func (r *Reconciler) ready(addon *v1alpha1.Addon) (ready v1.Condition) {
	ready = v1alpha1.Ready
	...
	for i := range addon.Status.Conditions {
		...
		if cnd.Type == v1alpha1.ValidationError {
			err = append(err, cnd.Message)
		}
	}
	if len(err) == 0 {
		ready.Status = v1.ConditionTrue
		ready.Reason = v1alpha1.Validated
		ready.Message = strings.Join(err, ";")
	} else {
		ready.Status = v1.ConditionFalse
		ready.Reason = v1alpha1.ValidationError
	}
	return
}

// addonChanged an addon has been created/updated.
func (r *Reconciler) addonChanged(addon *v1alpha1.Addon) (migrated bool, err error) {
