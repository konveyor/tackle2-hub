...
func TestTrackerCRUD(t *testing.T) {
	for _, r := range Samples {
		t.Run("Tracker "+r.Kind+" CRUD", func(t *testing.T) {
			...
			// Create a tracker.
			assert2.Must(t, Tracker.Create(&r))
			...
			assert2.Should(t, Tracker.Update(&r))
			...
			assert2.Must(t, Tracker.Delete(r.ID))
...
func TestTrackerList(t *testing.T) {
	...
	for _, createdTracker := range createdTrackers {
		...
		for _, retrievedTracker := range got {
			if assert2.FlatEqual(createdTracker.ID, retrievedTracker.ID) {
				found = true
				break
			}
		}
	...
	for _, tracker := range createdTrackers {
		...
		assert2.Must(t, Identity.Delete(tracker.Identity.ID))
	}
}
