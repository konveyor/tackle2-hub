...
func TestApplicationCreateGetDelete(t *testing.T) {
	...
	for _, r := range Samples {
		t.Run(r.Name, func(t *testing.T) {
			...
			// Create
			assert2.Should(t, Application.Create(&r))
			...
			if assert2.FlatEqual(foundR, r) {
				t.Errorf("Different list entry error. Got %v, expected %v", foundR, r)
			}

			// Try delete.
			assert2.Should(t, Application.Delete(got.ID))
...
func TestApplicationNotCreateDuplicates(t *testing.T) {
	...
	if err == nil {
		...
		// Clean the duplicate.
		assert2.Must(t, Application.Delete(dup.ID))
	}

	// Clean.
	assert2.Must(t, Application.Delete(r.ID))
}
