...
func TestReviewCRUD(t *testing.T) {
	for _, r := range Samples {
		t.Run("Review CRUD", func(t *testing.T) {
			...
			assert2.Must(t, Review.Create(&r))
			...
			assert2.Should(t, Review.Update(&r))
			...
			// Delete Review.
			assert2.Must(t, Review.Delete(r.ID))
		...
		t.Run("Copy Review", func(t *testing.T) {
			...
			assert2.Must(t, Application.Create(&srcApp))
			...
			assert2.Must(t, Application.Create(&destApp))
			...
			// Delete Review.
			assert2.Must(t, Review.Delete(r.ID))
			assert2.Must(t, Review.Delete(gotReview.ID))

			// Delete Applications.
			assert2.Must(t, Application.Delete(srcApp.ID))
			assert2.Must(t, Application.Delete(destApp.ID))
		})
	}
}
...
func TestReviewList(t *testing.T) {
	...
	for _, r := range Samples {
		...
		r.Application.ID = app.ID
		assert2.Should(t, Review.Create(&r))
	...
	for _, createdReview := range createdReviews {
		...
		for _, retrievedReview := range got {
			if assert2.FlatEqual(createdReview.ID, retrievedReview.ID) {
				found = true
				break
			}
		}
	...
	for _, review := range createdReviews {
		assert2.Must(t, Application.Delete(review.Application.ID))
		assert2.Must(t, Review.Delete(review.ID))
	}
}
