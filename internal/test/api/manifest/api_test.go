...
func TestManifestCRUD(t *testing.T) {
	...
	created := r
	if !assert2.MapEq(Base.Content, created.Content) {
		t.Errorf("Content mismatch.\n Expected: %s\n Actual: %s", Base.Content, created.Content)
	}
	if assert2.MapEq(Base.Secret, created.Secret) {
	...
	if !assert2.MapEq(Base.Secret, decrypted.Secret) {
	...
	if !assert2.MapEq(InjectedContent, injected.Content) {
