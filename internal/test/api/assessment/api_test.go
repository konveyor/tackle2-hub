...
func TestAssessmentCRUD(t *testing.T) {
	for _, r := range Samples {
		t.Run(fmt.Sprintf("%s for application %s", r.Questionnaire.Name, r.Application.Name), func(t *testing.T) {
			...
			assert2.Must(t, RichClient.Questionnaire.Create(&questionnaire))
			...
			if r.Application.Name != "" {
				...
				assert2.Must(t, RichClient.Application.Create(&app))
			...
			assert2.Must(t, RichClient.Application.Delete(r.Application.ID))
			assert2.Must(t, RichClient.Questionnaire.Delete(r.Questionnaire.ID))
		})
	}
}
