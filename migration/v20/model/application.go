...
type Application struct {
	...
	BinaryCoordinates string
	Facts             []Fact `gorm:"constraint:OnDelete:CASCADE"`
	Comments          string
	Tasks             []Task     `gorm:"constraint:OnDelete:CASCADE"`
	Tags              []Tag      `gorm:"many2many:application_tags"`
	Identities        []Identity `gorm:"many2many:application_identities;constraint:OnDelete:CASCADE"`
	BusinessServiceID *uint      `gorm:"index"`
	BusinessService   *BusinessService
	OwnerID           *uint         `gorm:"index"`
	Owner             *Stakeholder  `gorm:"foreignKey:OwnerID"`
	Contributors      []Stakeholder `gorm:"many2many:application_collaborators;constraint:OnDelete:CASCADE"`
...
func (ApplicationIdentity) TableName() string {
	return "application_identities"
}
...
func (r *Dependency) BeforeCreate(db *gorm.DB) (err error) {
	...
	for len(nextAppsIDs) != 0 {
		db.Where("to_id IN ?", nextAppsIDs).Find(&nextDeps)
...
type Stakeholder struct {
	...
	Groups           []StakeholderGroup `gorm:"many2many:stakeholder_group_stakeholders;constraint:OnDelete:CASCADE"`
	...
	Contributes      []Application   `gorm:"many2many:application_contributors;constraint:OnDelete:CASCADE"`
	MigrationWaves   []MigrationWave `gorm:"many2many:migration_wave_stakeholders;constraint:OnDelete:CASCADE"`
	Assessments      []Assessment    `gorm:"many2many:assessment_stakeholders;constraint:OnDelete:CASCADE"`
	Archetypes       []Archetype     `gorm:"many2many:archetype_stakeholders;constraint:OnDelete:CASCADE"`
}
...
type StakeholderGroup struct {
	...
	Username       string
	Description    string
	Stakeholders   []Stakeholder   `gorm:"many2many:stakeholder_group_stakeholders;constraint:OnDelete:CASCADE"`
	MigrationWaves []MigrationWave `gorm:"many2many:migration_wave_stakeholder_groups;constraint:OnDelete:CASCADE"`
	Assessments    []Assessment    `gorm:"many2many:assessment_stakeholder_groups;constraint:OnDelete:CASCADE"`
	Archetypes     []Archetype     `gorm:"many2many:archetype_stakeholder_groups;constraint:OnDelete:CASCADE"`
}
...
type MigrationWave struct {
	...
	Applications      []Application      `gorm:"constraint:OnDelete:SET NULL"`
	Stakeholders      []Stakeholder      `gorm:"many2many:migration_wave_stakeholders;constraint:OnDelete:CASCADE"`
	StakeholderGroups []StakeholderGroup `gorm:"many2many:migration_wave_stakeholder_groups;constraint:OnDelete:CASCADE"`
}
...
type Archetype struct {
	...
	Review            *Review            `gorm:"constraint:OnDelete:CASCADE"`
	Assessments       []Assessment       `gorm:"constraint:OnDelete:CASCADE"`
	CriteriaTags      []Tag              `gorm:"many2many:archetype_criteria_tags;constraint:OnDelete:CASCADE"`
	Tags              []Tag              `gorm:"many2many:archetype_tags;constraint:OnDelete:CASCADE"`
	Stakeholders      []Stakeholder      `gorm:"many2many:archetype_stakeholders;constraint:OnDelete:CASCADE"`
	StakeholderGroups []StakeholderGroup `gorm:"many2many:archetype_stakeholder_groups;constraint:OnDelete:CASCADE"`
